function M_DDFL_CT = data_driven_feedback_linearization(f_lifting, M_BILINEAR_CT, n, tol)
    
    n.controlled_states = 2;
    n.augmented_states = n.lifted_states+1;
    ...
    % Co = Co(:,1:(n.controlled_states-1)*(n.augmented_states));
    % [U, ~, ~] = svd(Co);
    % v = zeros(size(U,1),1); % column vector
    % for i=n.augmented_states:-1:1
    %     u = U(:,i); % this or this :U(:,i) or U(i,:)' ??
    %     % disp(norm(u'*A^(n.controlled_states-1)*B,1));
    %     % disp(u'*A^(n.controlled_states-1)*B);
    %     if norm(u'*A^(n.controlled_states-1)*B,1) >= tol
    %         v = u;
    %         break
    %     end
    % end

    v = zeros(n.augmented_states, 1);
    v(2) = 1;

    %% 3. Compute T, gamma and etta
    % M = v' ;
    % for i=1:n.controlled_states-1
    %     M = [M; v'*A^(i)];
    % end
    % T = @(q) M*phi(q);
    M = zeros(2, n.augmented_states);
    M(1,2) = 1; % To extract q1
    M(2,4) = 1; % To extract q3
    T = @(q) M*phi(q);
    v_gamma = v'*A^(n.controlled_states-1)*B;
    gamma = @(q) v_gamma*phi(q);
    v_etta = -v'*A^(n.controlled_states);
    etta = @(q) v_etta*phi(q);
