function [data, n] = structure_data(data_path, n, steps_length)
    ...
    for i = 1:n.trajs
        ...
        q = [q1 q2 q3 q4]';
        q = q(:, 2:end);        % remove first zero row due to reinitializing the arrays
        u = u';   
        u = u(:, 2:end);        % remove first zero row and last input we dont need it => NO We keep it now
    ...
    data.training.q = zeros(n.states, n.steps, n.trajs_training);
    data.training.u = zeros(n.inputs, n.steps, n.trajs_training);
    data.testing.q  = zeros(n.states, n.steps, n.trajs_testing);
