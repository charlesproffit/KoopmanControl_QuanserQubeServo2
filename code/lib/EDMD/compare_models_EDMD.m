function comparison = compare_models_EDMD(data_EDMD, f_lifting, M_BILINEAR_DT, M_BILINEAR_CT, M_EDMD, M_LINEAR, n, Ts)
    q_nonlin = data_EDMD.testing.q;
    q_edmdbilin_DT = zeros(n.lifted_states, n.steps, n.trajs_testing);
    q_edmdbilin_CT = zeros(n.lifted_states, n.steps, n.trajs_testing);
    q_edmdlin      = zeros(n.lifted_states, n.steps, n.trajs_testing);
    q_lin          = zeros(n.states,        n.steps, n.trajs_testing);
    ...
    for i = 1:n.trajs_testing
        ...
        for j = (1:n.steps-1)
            ...
            q_edmdbilin_DT(:,j+1,i) = M_BILINEAR_DT.A*z + M_BILINEAR_DT.N*(z.*ui);


            % Bilinear CT (Euler integration)
            z = q_edmdbilin_CT(:,j,i);
            zdot = M_BILINEAR_CT.A*z + M_BILINEAR_CT.N*(z.*ui);
    ...
    for i = 1:n.trajs_testing
        E_bilin_DT = comparison.q_BILINEAR_DT(2:n.states+1,:,i) - comparison.q_nonlinear(:,:,i);
        E_bilin_CT = comparison.q_BILINEAR_CT(2:n.states+1,:,i) - comparison.q_nonlinear(:,:,i);
        E_edmd     = comparison.q_EDMD(2:n.states+1,:,i)        - comparison.q_nonlinear(:,:,i);
    ...
    for i = 1:n.trajs_testing
        lifted_true = zeros(n.lifted_states, n.steps);
        for t = 1:n.steps
    ...
    comparison.svds.LINEAR = M_LINEAR.svds;
    comparison.svds.EDMD = M_EDMD.svds;
    comparison.svds.BILINEAR_DT = M_BILINEAR_DT.svds;
    comparison.svds.BILINEAR_CT = M_BILINEAR_CT.svds;

    comparison.cond.LINEAR = M_LINEAR.condition_number;
    comparison.cond.EDMD = M_EDMD.condition_number;
    comparison.cond.BILINEAR_DT = M_BILINEAR_DT.condition_number;
    comparison.cond.BILINEAR_CT = M_BILINEAR_CT.condition_number;
